...
class ValueDependencyEdge<F extends Object, T extends Object> extends Edge<F,T> {
...
class DerivedListenableEdge<F extends Object, T extends Object> extends Edge<F,T> {
...
class ListenableService<T extends Listenable> extends Service<T> {
	...
	void notifyListeners() {
		...
		if (_value case ChangeNotifier notifier) {
			// HACK: do something else
			print("forward notify $T");
...
class ServiceGraph {
	...
	void addListenableDep<F extends Object, T extends Listenable>(FutureOr<T> Function(F) f) {
	...
	void deriveStream<F extends Object, T extends Object>(Stream<T> Function(F) f) {
		var node = _services[T] = Service<T>(this, null);
		var from = _services[F]! as Service<F>;
	...
	void deriveListenable<F extends Object, T extends Object>(ValueListenable<T> Function(F) f) {
		var node = _services[T] = Service<T>(this, null);
		var from = _services[F]! as Service<F>;
...
class ServiceGraphProvider extends StatelessWidget {
	...
	const ServiceGraphProvider.value({super.key, required this.graph, required this.child});
