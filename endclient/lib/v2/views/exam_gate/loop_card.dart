...
class LoopCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      children: [
        header(),
        grid(),
        if (remarks.isNotEmpty) remarksList(remarks),
      ],
    ));
  }

  static Widget preExam(VetData data) => Card(
        elevation: 0,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
  ...
  static Widget remarksList(List<VetFieldValue> remarks,
      ...
      Container(
          ...
          child: Wrap(
            ...
            children: [
              ...
              if (remarks.isEmpty)
                const Chip(
                    backgroundColor: Colors.green, label: Text("No remarks!"))
            ],
          ));
          ...
          return AspectRatio(
            ...
            child: GridView.count(
              ...
              children: [
                ...
                txtCol([
                  "${loopData.data?.hr1 ?? "-"}/${loopData.data?.hr2 ?? "-"}",
                  "Heartrate"
                ]),
                txtCol([
                  maybe(loopData.speed(finish: isFinish)?.toStringAsFixed(1),
                          (s) => "$s km/h") ??
                      "-",
                  "Speed"
                ]),
                txtCol([
                  maybe(loopData.expDeparture, unixHMS) ?? "-",
                  "Departure"
                ]),
...
Widget txtCol(List<String> strs) => Column(
      children: strs.map((s) => Text(s)).toList(),
    );

Widget wrapTxtCol(Widget w) => AspectRatio(
    aspectRatio: 1,
    child: Container(
